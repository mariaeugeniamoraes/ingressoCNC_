...
GEOMETRIA = {
    "social": {
        "nome": "Social", "cor": "#2b3a8f",
        "inicio": 214, "fim": 312,
        "portoes": [("64", 222, 26), ("A", 228, 58), ("63", 292, 26)],
    },
    "caldeirao": {
        "nome": "Caldeirão", "cor": "#1f9a55",
        "inicio": 314, "fim": 374,
        "portoes": [],
    },
    "visitante": {
        "nome": "Visitante", "cor": "#f08a24",
        "inicio": 376, "fim": 418,
        "portoes": [("68", 40, 26)],
    },
    "vermelho": {
        "nome": "Vermelho", "cor": "#d7191c",
        "inicio": 64, "fim": 120,
        "portoes": [("67", 88, 26), ("D", 83, 58), ("C", 97, 58)],
    },
    "hexa": {
        "nome": "Hexa", "cor": "#f2d21b",
        "inicio": 136, "fim": 212,
        "portoes": [("B", 191, 60), ("65", 196, 26), ("66", 184, 26)],
    },
}
...
def _ponto(angulo, rx, ry):
    ...
    return CENTRO[0] + rx * math.cos(t), CENTRO[1] + ry * math.sin(t)
...
def _caminho(inicio, fim):
    ...
    return (
        f"M {x1:.1f} {y1:.1f} A {rx} {ry} 0 {grande} 1 {x2:.1f} {y2:.1f} "
        f"L {x3:.1f} {y3:.1f} A {irx} {iry} 0 {grande} 0 {x4:.1f} {y4:.1f} Z"
    )


def _cor_do_texto(cor_hex):
    """Texto escuro em cores claras (ex.: amarelo), branco nas escuras."""
...
def montar_mapa(setores):
    ...
    """
    do_banco = {slugify(s.nome): s for s in setores}
    desenho = []
    faltando = []
    ...
    for chave, geo in GEOMETRIA.items():
        ...
        if setor is None:
            ...
            situacao = _formatar_preco(setor.preco) if disponivel else "Esgotado"
            quantidade = setor.quantidade
            setor_id = setor.id
            nome = setor.nome
            preco = _formatar_preco(setor.preco)

        cor = geo["cor"] if disponivel else COR_INDISPONIVEL

        meio = (geo["inicio"] + geo["fim"]) / 2
        rotulo_x, rotulo_y = _ponto(
            meio,
            (RAIO_EXTERNO[0] + RAIO_INTERNO[0]) / 2,
            (RAIO_EXTERNO[1] + RAIO_INTERNO[1]) / 2,
        )

        portoes = []
        for rotulo, angulo, distancia in geo["portoes"]:
            x, y = _ponto(angulo, RAIO_EXTERNO[0] + distancia, RAIO_EXTERNO[1] + distancia)
            portoes.append({
                "rotulo": rotulo,
                "x": round(x, 1),
                "y": round(y, 1),
        ...
        desenho.append({
            ...
            "caminho": _caminho(geo["inicio"], geo["fim"]),
    ...
    dados_js = {
        s["chave"]: {
            ...
            "quantidade": do_banco[s["chave"]].quantidade if s["id"] else 0,
            "portoes": ", ".join(p["rotulo"] for p in s["portoes"]) or "Consulte o clube",
        }
        for s in desenho
    }
